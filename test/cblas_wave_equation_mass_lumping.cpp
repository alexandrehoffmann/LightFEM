...
int main()
{
	...
	//////////////////////////////////////////////////////////////////////
	////  We discretize our PDE using the gls implementation of blas  ////
	////         Note that any Linear algebra could be used.          ////
	//////////////////////////////////////////////////////////////////////
	
	// mass matrix M_ij = $m(u_j,v_i)$
	gsl_vector* M = gsl_vector_calloc(Vh.getNBasisFunction());
	for (size_t idx=0;idx<m.getNCoefs();++idx)
	{
		const auto& [i,j,mij] = m.getCoef(idx);
		if (i == j) { gsl_vector_set(M, i, mij); }
	}
	
	// stiffness matrix K_ij = $a(u_j,v_i)$
	gsl_spmatrix* tmpK = gsl_spmatrix_alloc_nzmax(Vh.getNBasisFunction(), Vh.getNBasisFunction(), a.getNCoefs(), GSL_SPMATRIX_TRIPLET);
	for (size_t idx=0;idx<a.getNCoefs();++idx)
	{
		auto&& [i,j,aij] = a.getCoef(idx);
		gsl_spmatrix_set(tmpK, i, j, aij);
	}
	gsl_spmatrix* K = gsl_spmatrix_crs(tmpK);
	gsl_spmatrix_free(tmpK);
	
	gsl_vector* f = gsl_vector_alloc(Vh.getNBasisFunction()); // source term f_i = $l(v_i)$
	for (size_t i=0;i<l.getNCoefs();++i)
	{
		gsl_vector_set(f, i, l[i]); // l[i] returns l(v_i)
	}
	
	// Compute the inverse of our mass matrix
	gsl_vector* invSemM = gsl_vector_alloc(Vh.getNBasisFunction());
	for (size_t i=0;i<Vh.getNBasisFunction();++i)
	{
		gsl_vector_set(invSemM, i, 1.0 / gsl_vector_get(M, i));
	}
	
	gsl_vector* uk = gsl_vector_calloc(Vh.getNBasisFunction()); // wavefield $u(t_k)$
	gsl_vector* vk = gsl_vector_calloc(Vh.getNBasisFunction()); // wavefield velocity $\dot{u}(t_k)$
	gsl_vector* ak = gsl_vector_calloc(Vh.getNBasisFunction()); // wavefield acceleration $\ddot{u}(t_k)$
	
	gsl_vector* fk = gsl_vector_alloc(Vh.getNBasisFunction()); // source term $f(t_k)$
	gsl_vector* ukp12 = gsl_vector_alloc(Vh.getNBasisFunction()); // $uk + dt*vk + 0.5*dt*dt*ak$
	...
	for (size_t k=0;k<nt+1;++k)
	{
		...
		FiniteElementScalarField u(&Vh);
		for (size_t i=0;i<Vh.getNBasisFunction();++i) { u.setCoef(i, gsl_vector_get(uk, i)); } 
		if (k % 100 == 0) // print the function every 100 iterations
		{
			printFunction("gsl_sem_u_" + std::to_string(idx) + ".dat", u.discretize());
			++idx;
		}
		
		for (size_t i=0;i<Vh.getNBasisFunction();++i)
		{
			gsl_vector_set(fk, i, ricker(t)*gsl_vector_get(f, i));
			gsl_vector_set(ukp12, i, gsl_vector_get(uk,i) + dt*gsl_vector_get(vk,i) +  + 0.5*dt*dt*gsl_vector_get(ak, i) );
		}
		
		gsl_spblas_dgemv(CblasNoTrans, -1.0, K, ukp12, 1.0, fk); // fk = -K ukp12 + fk
		
		for (size_t i=0;i<Vh.getNBasisFunction();++i)
		{
			const double akp1i = gsl_vector_get(invSemM, i)*gsl_vector_get(fk, i);
			const double vkp1i = gsl_vector_get(vk, i) + (1.0 - gamma)*dt*gsl_vector_get(ak, i) + gamma*dt*akp1i;
			const double ukp1i = gsl_vector_get(uk, i) + dt*gsl_vector_get(vk, i) + 0.5*dt*dt*gsl_vector_get(ak, i);
			
			gsl_vector_set(uk, i, ukp1i);
			gsl_vector_set(vk, i, vkp1i);
			gsl_vector_set(ak, i, akp1i);
		}
