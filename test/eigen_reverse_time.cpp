...
{	
	...
	for (size_t k=0;k<nt+1;++k)
	{
		...
		if (k % 10 == 0) // print the function every 100 iterations
		{
			...
			//FiniteElementScalarField dot_u(&Vh, reinterpret_cast< Scalar* >(vk.data()));
			//FiniteElementScalarField ddot_u(&Vh, reinterpret_cast< Scalar* >(ak.data()));
			
			printFunction("u_" + std::to_string(idx) + ".dat", u.discretize());
			//printFunction("v_" + std::to_string(idx) + ".dat", dot_u.discretize());
			//printFunction("a_" + std::to_string(idx) + ".dat", ddot_u.discretize());
			++idx;
		}
		if (k < nt)
		{
