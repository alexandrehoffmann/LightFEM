...
int main()
{
	...
	// create a bilinear form $m(u,v) := \int_\Omega uv dx$
	BilinearForm m(&Vh, &Vh, [&mesh, &c](const TrialFunction& u, const TestFunction& v) -> double
	{
	...
	M.setFromTriplets(reinterpret_cast< const Eigen::Triplet<double>* >(m.getCoefData()), reinterpret_cast< const Eigen::Triplet<double>* >(m.getCoefData() + m.getNCoefs()));
	...
	// Compute the Cholesky factorization of our mass matrix
	Eigen::SparseLU<Eigen::SparseMatrix< double > > LLt(M);
	...
	for (size_t k=0;k<nt+1;++k)
	{
		...
		const Eigen::VectorXd akp1 = LLt.solve( ricker(t)*f - K*(uk + dt*vk + 0.5*dt*dt*ak) );
