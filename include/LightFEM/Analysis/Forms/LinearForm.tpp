...
#ifndef LINEAR_FORM_TPP
#define LINEAR_FORM_TPP
...
LinearForm::LinearForm(const FunctionSpace *Vh, const Form form) :
	m_Vh(Vh),
...
template<typename Form>
LinearForm::LinearForm(const FunctionSpace *Vh, Form form, MPI_Comm com) : 
	m_Vh(Vh),
	m_coefs(Vh->getNBasisFunction(), 0.0)
{
...
template<typename Form>
CpxLinearForm::CpxLinearForm(const FunctionSpace *Vh, Form form) :
	m_Vh(Vh),
	m_coefs(Vh->getNBasisFunction(), 0.0)
{
	#pragma omp parallel
...
template<typename Form>
CpxLinearForm::CpxLinearForm(const FunctionSpace *Vh, Form form, MPI_Comm com) : 
	m_Vh(Vh),
	m_coefs(Vh->getNBasisFunction(), 0.0)
{
